    ...
    if (isDev) {
        ...
        const loadApp = () => {
            if (mainWindow) {
                ...
                // DevTools are no longer opened automatically - users can open them manually if needed
...
ipcMain.handle('get-file-path', async (_event, filePath) => {
...
ipcMain.handle('ensure-directory-exists', async (_event, directory) => {
    try {
...
ipcMain.handle('save-screenshot', async (_event, filename, buffer, directory) => {
    try {
...
// Save CSV to file
ipcMain.handle('save-csv', async (_event, filename, buffer, directory) => {
    try {
        // Use provided directory or fall back to default
        const targetDir = directory || getAnnotationsDirectory();
        // Ensure directory exists
...
ipcMain.handle('add-recent-project', async (_event, filePath, name) => {
    try {
...
ipcMain.handle('check-for-project-file', async (_event, videoPath) => {
    try {
...
ipcMain.handle('capture-window-screenshot', async (_event, rect) => {
    try {
        if (!mainWindow) {
...
// Save PDF file
ipcMain.handle('save-pdf', async (_event, filename, buffer, directory) => {
    try {
        // Use provided directory or fall back to default
        const targetDir = directory || getAnnotationsDirectory();
        // Ensure directory exists
...
// Save PDF to Downloads and open it
ipcMain.handle('save-and-open-pdf', async (_event, filename, buffer) => {
    try {
        ...
        const bufferToWrite = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer);
        await writeFile(filePath, bufferToWrite);
        console.log('PDF saved to Downloads:', filePath);
        // Open the PDF with the system's default PDF viewer
        await shell.openPath(filePath);
        return { success: true, path: filePath };
    }
    catch (error) {
        console.error('Failed to save and open PDF:', error);
        throw error;
    }
});
// Save video file
ipcMain.handle('save-video', async (_event, filename, buffer, directory) => {
    try {
        // Use provided directory or fall back to default
        const targetDir = directory || getAnnotationsDirectory();
        // Ensure directory exists
