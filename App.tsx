...
import { loadAnnotations, saveAnnotations, saveScreenshotToFile, saveAnnotationsToCSV, saveProjectFile, loadProjectFile, ensureDeliverablesFolder, getDirectoryFromPath } from './utils/storage';
import { generatePDFReport } from './utils/pdfReport';
import { generateInstructionManual } from './utils/instructionManual';
...
function App() {
  ...
  // Autosave feature: Save screenshots, CSV, and project file every 3 minutes
  useEffect(() => {
    // Only run autosave in Electron mode (browser mode would trigger downloads)
    if (!window.electronAPI) {
      return;
    }

    // Only run autosave if we have a video path and annotations
    if (!videoPath || annotations.length === 0) {
      return;
    }

    // Get the root folder (directory where video is located)
    const rootFolder = getDirectoryFromPath(videoPath);
    if (!rootFolder) {
      console.warn('Cannot autosave: Unable to determine root folder from video path');
      return;
    }
    ...
    const performAutosave = async () => {
      try {
        ...
        // Ensure the root folder exists (should already exist, but just in case)
        if (window.electronAPI && window.electronAPI.ensureDirectoryExists) {
          await window.electronAPI.ensureDirectoryExists(rootFolder);
        }
          ...
          if (annotation.screenshotDataUrl) {
            try {
              await saveScreenshotToFile(
                ...
                annotation.id,
                videoFileName,
                rootFolder // Save to root folder, not deliverables
              );
              screenshotsSaved++;
            } catch (error) {
              console.error(`Autosave: Failed to save screenshot for annotation ${annotation.id}:`, error);
        ...
        // 2. Save CSV file
        try {
          await saveAnnotationsToCSV(annotations, videoFileName, rootFolder);
          console.log('Autosave: CSV saved successfully');
        } catch (error) {
          console.error('Autosave: Failed to save CSV:', error);
        }

        // 3. Save project file
        try {
          await saveProjectFile(annotations, videoFileName, videoPath);
          console.log('Autosave: Project file saved successfully');
        } catch (error) {
          console.error('Autosave: Failed to save project file:', error);
        }

        console.log(`Autosave: Completed successfully (${screenshotsSaved} screenshots, CSV, and project file saved)`);
      } catch (error) {
        console.error('Autosave: Error during autosave:', error);
        // Don't show alert to user - autosave should be silent
      }
    };

    // Set up interval: 3 minutes = 180000 milliseconds
    const autosaveInterval = setInterval(performAutosave, 3 * 60 * 1000);

    // Perform initial autosave after 3 minutes (don't save immediately on load)
    // This gives the user time to start working before first autosave

    // Cleanup interval on unmount or when dependencies change
    return () => {
      clearInterval(autosaveInterval);
    };
  }, [videoPath, annotations, videoFileName]);

  // Note: Screenshots and CSV are now automatically saved every 3 minutes via autosave
  // Manual export is still available via File menu
  ...
  const handleSaveEditedAnnotation = useCallback((updatedAnnotation: Annotation) => {
    setAnnotations((prev) => 
  ...
  // Track the annotation being edited
  ...
  // Handle open instruction manual
  const handleOpenInstructionManual = async () => {
    try {
      await generateInstructionManual();
    } catch (error) {
      console.error('Failed to generate instruction manual:', error);
      alert(`Failed to open instruction manual: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <h1 style={{ margin: 0, fontSize: '20px' }}>OmnInspect</h1>
